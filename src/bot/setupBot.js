...
const { hasAdminAccess, requireAdmin } = require('../middleware/auth');
...
const { buildDashboardReport } = require('../services/dashboardReportService');
const { clearTrackedChat } = require('../jobs/chatCleanupJob');
...
async function getRandomAdminContact() {
  return adminService.getRandomContactAdmin();
}

async function adminChatButtonRow() {
  const admin = await getRandomAdminContact();
  if (!admin) return null;
  return [Markup.button.url(`📞 ${admin.label}`, admin.url)];
}

async function resolveCurrentRole(ctx) {
  if (await hasAdminAccess(ctx.from?.id)) return 'admin';
...
async function openRequestsPanel(ctx) {
  const [pending, approved, rejected] = await Promise.all([
    rentalRequestService.listRequests('pending'),
    rentalRequestService.listRequests('approved'),
    rentalRequestService.listRequests('rejected')
  ]);

  return renderPanel(
    ctx,
    `📩 REQUEST MANAGEMENT\n━━━━━━━━━━━━━━━━━━\nYou have ${pending.length} requests awaiting your review.`,
    getRequestMenu({
...
async function getRoomMenuStats() {
  const rooms = await roomService.listRooms();
  return {
    totalRooms: rooms.length,
    freeRooms: rooms.filter((room) => room.status === 'free').length,
    rentedRooms: rooms.filter((room) => room.status === 'rented').length
  };
}

function getRoomPaginationRow(prefix, page, totalPages) {
  if (totalPages <= 1) {
    return [Markup.button.callback('📄 Page 1 / 1', 'noop')];
  }

  return [
    Markup.button.callback(page > 1 ? '⬅️ Prev' : '·', `${prefix}:${Math.max(1, page - 1)}`),
    Markup.button.callback(`📄 ${page} / ${totalPages}`, 'noop'),
    Markup.button.callback(page < totalPages ? 'Next ➡️' : '·', `${prefix}:${Math.min(totalPages, page + 1)}`)
  ];
}

function getRoomPaymentMarker(room, payment) {
  if (room.status !== 'rented') return '✅';
  if (!payment) return '💸';
  if (payment.status === 'overdue') return '⚠️';
  if (payment.status === 'unpaid') return '💸';
  return '✅';
}

async function openRoomsPanel(ctx) {
  const stats = await getRoomMenuStats();
  return renderPanel(
    ctx,
...
async function showRoomList(ctx, type = 'all', page = 1) {
  ...
  if (!rooms.length) return renderPanel(ctx, '🏠 Room Management\nNo rooms available yet.', getRoomsMenu(await getRoomMenuStats()));

  const p = paginate(rooms, page, 6);
  const buttons = p.data.map((room) => Markup.button.callback(`${room.status === 'rented' ? '🔴' : '🟢'} ${room.roomNumber}`, callback(['room', 'view', room._id])));
  const rows = chunkTwoColumns(buttons);
  rows.push(getRoomPaginationRow(callback(['rooms', 'list', type]), p.currentPage, p.totalPages));
  rows.push([Markup.button.callback('🔙 Back to Menu', 'panel:rooms')]);

  const stats = await getRoomMenuStats();
  const title = type === 'free'
    ? '🟢 AVAILABLE ROOMS'
    : type === 'rented'
      ? '🔴 RENTED UNITS'
      : '🏨 ROOM LISTING';
  const summary = type === 'free'
    ? `${stats.freeRooms} units ready for move-in`
    : type === 'rented'
      ? `${stats.rentedRooms} Active Tenancies`
      : `🟢 ${stats.freeRooms} Free • 🔴 ${stats.rentedRooms} Rented`;

  return renderPanel(ctx, `${title}\n${summary}\n━━━━━━━━━━━━━`, { reply_markup: { inline_keyboard: rows } });
}
...
async function showAvailableRooms(ctx, page = 1) {
  ...
  const p = paginate(rooms, page, 6);
  const buttons = p.data.map((room) => Markup.button.callback(`🟢 ${room.roomNumber} | ${formatMoney(room.rentPrice)}`, callback(['guest', 'room', room._id])));
  const rows = chunkTwoColumns(buttons);
  rows.push(getRoomPaginationRow(callback(['guest', 'available']), p.currentPage, p.totalPages));
  rows.push([Markup.button.callback('➕ Register New Tenant', 'guest:rent:start')]);
  rows.push([Markup.button.callback('🔙 Rooms Menu', 'panel:home')]);
  return renderPanel(ctx, `🟢 AVAILABLE ROOMS\n${rooms.length} units ready for move-in\n━━━━━━━━━━━━━`, { reply_markup: { inline_keyboard: rows } });
}
...
async function showRentedRoomList(ctx, page = 1) {
  const rooms = await roomService.listRooms({ status: 'rented' });
  if (!rooms.length) return renderPanel(ctx, '🔴 RENTED UNITS\nNo active tenancies.', { reply_markup: { inline_keyboard: [[Markup.button.callback('🔙 Rooms Menu', 'panel:rooms')]] } });

  const p = paginate(rooms, page, 6);
  const roomEntries = await Promise.all(p.data.map(async (room) => {
    const payment = await paymentService.getUnpaidForRoom(room._id);
    return Markup.button.callback(`🏠 Room ${room.roomNumber} ${getRoomPaymentMarker(room, payment)}`, callback(['room', 'view', room._id]));
  }));
  const rows = chunkTwoColumns(roomEntries);
  rows.push([Markup.button.callback('────────', 'noop')]);
  rows.push([
    Markup.button.callback(`📄 ${p.currentPage} / ${p.totalPages}`, 'noop'),
    Markup.button.callback('🔍 Find Room', 'rooms:search:start')
  ]);
  rows.push([
    Markup.button.callback('💰 Unpaid Rent', 'pay:list:unpaid:1'),
    Markup.button.callback('🔙 Rooms Menu', 'panel:rooms')
  ]);

  return renderPanel(ctx, `🔴 RENTED UNITS\n${rooms.length} Active Tenancies\n━━━━━━━━━━━━━\n✅ Paid  💸 Unpaid  ⚠️ Overdue`, { reply_markup: { inline_keyboard: rows } });
}
...
function setupBot() {
  ...
  bot.use(async (ctx, next) => {
    if (ctx.from) {
      try {
        await adminService.syncAdminIdentity(ctx.from);
      } catch (error) {
        console.error('Admin identity sync failed:', error.message);
      }
    }
    return next();
  });
  ...
  bot.use(async (ctx, next) => {
    ...
    const recordOutgoing = async (sent) => {
      if (!sent?.message_id) return sent;
      try {
        await chatHistoryService.recordChatActivity({
          chatId: sent.chat?.id || chatId,
          role: await resolveCurrentRole(ctx),
          messageId: sent.message_id
        });
      } catch (error) {
        console.error('Chat history outgoing sync failed:', error.message);
      }
      return sent;
    };

    const originalReply = ctx.reply.bind(ctx);
    ctx.reply = async (...args) => recordOutgoing(await originalReply(...args));

    const originalReplyWithPhoto = ctx.replyWithPhoto.bind(ctx);
    ctx.replyWithPhoto = async (...args) => recordOutgoing(await originalReplyWithPhoto(...args));

    const originalReplyWithDocument = ctx.replyWithDocument.bind(ctx);
    ctx.replyWithDocument = async (...args) => recordOutgoing(await originalReplyWithDocument(...args));
    ...
    if (ctx.message?.message_id) {
      try {
        ...
        console.error('Chat history incoming sync failed:', error.message);
      }
    }
  ...
  bot.hears('🏠 My Room', async (ctx) => {
    ...
    const payment = await paymentService.getTenantCurrentPayment(tenant._id);
    const adminChatRow = await adminChatButtonRow();
    const actions = Markup.inlineKeyboard([
      [Markup.button.callback('💳 My Payment', 'tenant:mypayment')],
      adminChatRow || [Markup.button.callback('📞 Contact Admin', 'tenant:contact')],
  ...
  bot.hears('📞 Contact Admin', async (ctx) => {
    const admin = await getRandomAdminContact();
    if (admin) {
      return ctx.reply(`Support staff: ${admin.label}`, Markup.inlineKeyboard([[Markup.button.url('💬 Chat Staff', admin.url)]]));
    }
    return ctx.reply('Support is available. Please contact building management.');
  });
  bot.hears('📍 View Location', async (ctx) =>
    ctx.reply(
      '📍 Red House Location',
      Markup.inlineKeyboard([[Markup.button.url('🗺 Open in Maps', MAP_URL)]])
    ));
    ...
    try {
      ...
      if (data === 'flow:submitrequest') {
        if (ctx.session.flow === 'rental_request' && ctx.session.step === 'confirm') {
          ...
          const successRows = [];
          const admin = await getRandomAdminContact();
          if (admin) successRows.push([Markup.button.url(`📞 ${admin.label}`, admin.url)]);
      ...
      if (data === 'dashboard:download') {
        const admin = await adminService.getAdminByTelegramId(ctx.from.id);
        const requestedBy = admin?.fullName
          || (admin?.telegramUsername ? `@${admin.telegramUsername}` : null)
          || (ctx.from?.username ? `@${ctx.from.username}` : null)
          || `${ctx.from?.first_name || 'Admin'} (${ctx.from?.id || '-'})`;
        const report = await buildDashboardReport({ requestedBy });
        await ctx.replyWithDocument({ source: report.buffer, filename: report.filename });
        return ctx.answerCbQuery('Excel downloaded');
      }
      if (data === 'tenant:mypayment') return ctx.reply('Tap 💳 My Payment from main menu.');
      if (data === 'tenant:contact') {
        const admin = await getRandomAdminContact();
        if (admin) {
          return safeEditOrReply(ctx, `Support staff: ${admin.label}`, { reply_markup: { inline_keyboard: [[Markup.button.url('💬 Chat Staff', admin.url)]] } });
        }
      ...
      if (scope === 'rooms' && action === 'list') {
        if (p1 === 'rented') return showRentedRoomList(ctx, Number(p2 || 1));
        return showRoomList(ctx, p1, Number(p2 || 1));
      }
      if (scope === 'guest' && action === 'available') return showAvailableRooms(ctx, Number(p1 || 1));
      if (scope === 'guest' && action === 'rent' && p1 === 'start') {
        startFlow(ctx, 'tenant_link', 'room');
        return safeEditOrReply(ctx, `${getSectionHeader('Register My Room', '📝')}\nEnter your room number.`, { reply_markup: { inline_keyboard: [[Markup.button.callback('❌ Cancel', 'flow:cancel')]] } });
      }
      ...
      if (scope === 'rooms' && action === 'search') {
        ...
        return safeEditOrReply(ctx, '🔍 Search Room\n━━━━━━━━━━━━━\n\nPlease enter a room number like A01.', {
          reply_markup: { inline_keyboard: [[Markup.button.callback('⬅️ Back to Rooms', 'panel:rooms')], [Markup.button.callback('🏠 Main Menu', 'panel:home')]] }
        });
      }
      ...
      if (scope === 'request' && action === 'approve') {
        ...
        try {
          ...
          return safeEditOrReply(ctx, `Request approved, but auto-link failed: ${error.message}`, {
            reply_markup: { inline_keyboard: [[Markup.button.callback('✅ Assign Now', callback(['request', 'assign', req._id])), Markup.button.callback('🔙 Back', callback(['request', 'view', req._id]))]] }
          });
        }

        if (req.telegramChatId) {
          await ctx.telegram.sendMessage(req.telegramChatId, `✅ Your request for room ${req.roomNumber} was approved. You can now open My Room.`);
        }
        return safeEditOrReply(ctx, `Request approved and ${req.fullName} was linked to room ${req.roomNumber}.`);
      }
      ...
      if (scope === 'settings' && action === 'admins' && p1 === 'list') {
        ...
        const rows = admins.map((a) => {
          ...
          const roleLabel = adminService.isDeveloperAdmin(a) ? 'Developer' : (a.roleId?.name || '-');
          return [Markup.button.callback(`${displayName} • ${roleLabel}`, callback(['settings', 'admins', 'view', a._id]))];
        });
      ...
      if (scope === 'settings' && action === 'admins' && p1 === 'view') {
        ...
        const roleLabel = adminService.isDeveloperAdmin(admin) ? 'Developer' : (admin.roleId?.name || '-');
        const profileRows = [
          [Markup.button.callback('✏️ Edit Name', callback(['settings', 'admins', 'editname', admin._id]))],
          [Markup.button.callback('✏️ Change Role', callback(['settings', 'admins', 'changerole', admin._id]))]
        ];
        if (!adminService.isDeveloperAdmin(admin)) {
          profileRows.push([Markup.button.callback('❌ Remove Admin', callback(['settings', 'admins', 'remove', admin._id]))]);
        }
        profileRows.push([Markup.button.callback('🔙 Back', 'settings:admins:list')]);
        return renderPanel(
          ctx,
          ...
          { reply_markup: { inline_keyboard: profileRows } }
        );
      }
      ...
      if (scope === 'settings' && action === 'reminder' && p1 === 'clearchat') {
        if (!(await adminService.hasPermission(ctx.from.id, 'run_reminders'))) return ctx.answerCbQuery('No permission');
        await ctx.answerCbQuery('Clearing chat...');
        await clearTrackedChat(bot, ctx.chat?.id);
        return;
      }
